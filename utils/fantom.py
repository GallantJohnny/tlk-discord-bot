...
@logger.catch
def verify_address(w3, address, key):
    with open("abi/verifier.abi.json") as f:
        verifier_abi = json.load(f)
    verifier = w3.eth.contract(
                    address="0x1051e32767Ca540B46aeE987ff54F63B59ca8581",
                    abi=verifier_abi
                )
    hash_ = verifier.functions.getKeyHash(address).call()
    if hash_ == w3.solidityKeccak(["address", "string"], [address, key]):
        return True
    return False
